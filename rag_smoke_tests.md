...
메인 컬렉션 `pdf_chunks` 기준으로 점검한다.

포트폴리오 공개본에서는 실제 원문과 색인 DB를 포함하지 않는다. 아래 질문은 내부 문서를 그대로 공개하기 위한 자료가 아니라, 검색 품질을 어떤 관점으로 점검했는지 설명하기 위한 예시다.

## 기본 원칙

- 문서 원문을 대화에 붙이지 않는다
- 질문은 실제 현장에서 할 법한 표현으로 작성한다
- 결과 평가는 `정답 문서가 상위에 오는지`, `출처가 읽기 쉬운지`, `근거 밖 추론을 과하게 하지 않는지`를 본다
...
- RAG 실사용 점검: `rag_ollama_answer.py`
  - 기본값 `top-k=5` 사용
  - 특정 문서 범위만 검색하려면 `--category "검사/검체"`처럼 지정한다
...
- 1순위 또는 2순위 안에 핵심 문서가 나온다
- 출처가 실제 PDF 파일명으로 표시된다
- 서로 같은 페이지 중복이 과도하지 않다
- 답변이 문서 근거를 벗어나 과장되지 않는다

주의할 결과:

- 1순위 문서가 아예 다른 주제다
- 문서명이 묶음명으로 나오거나 출처가 불명확하다
- 같은 문서 같은 페이지 청크만 반복된다
- 현장형 질문에 일반론만 길게 답한다
