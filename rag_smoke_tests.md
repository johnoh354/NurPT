...
아래 질문은 상황 기반 질문과 절차 기반 질문을 섞어 검색 품질을 확인한 예시입니다.

- `내 환자 의식 사정은 어떻게 하면 될까? 잠을 자고 있는 것 같아서.`
- `섬망 선별 검사는 언제 어떤 도구로 보면 될까? 지금 저녁인데.`
- `혈당 측정 기록 절차는 어디 문서에서 확인하면 될까?`
- `유치도뇨관 삽입 후 간호는 어느 문서를 보면 될까?`
- `경장영양 펌프 관련 간호는 어디에서 찾으면 될까?`
...
```bash
python3 rag_ollama_answer.py "혈당 측정 기록 절차는 어디 문서에서 확인하면 될까?" --db-path "./chroma_db" --collection "pdf_chunks"
```
